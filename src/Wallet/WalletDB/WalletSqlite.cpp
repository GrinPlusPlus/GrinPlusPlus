...
WalletSqlite::WalletSqlite(const Config &config) : m_config(config)
{
}
...
bool WalletSqlite::OpenWallet(const std::string &username, const SecureVector &masterSeed)
{
    ...
    if (FileUtil::Exists(dbFile))
    {
        ...
        {
            LoggerAPI::LogError("WalletSqlite::OpenWallet - Failed to open wallet.db for user: " + username +
                                ". Error: " + std::string(sqlite3_errmsg(pDatabase)));
...
KeyChainPath WalletSqlite::GetNextChildPath(const std::string &username, const KeyChainPath &parentPath)
{
    ...
    {
        LoggerAPI::LogError(StringUtil::Format("WalletSqlite::GetNextChildPath - Error while compiling sql: %s",
    ...
    {
        LoggerAPI::LogError(StringUtil::Format("WalletSqlite::GetNextChildPath - Error finalizing statement: %s",
                                               sqlite3_errmsg(pDatabase)));
    ...
    const std::string update =
        "UPDATE accounts SET next_child_index=" + std::to_string(nextChildPath.GetKeyIndices().back() + 1) +
        " WHERE parent_path='" + parentPath.ToString() + "';";
    if (sqlite3_exec(pDatabase, update.c_str(), NULL, NULL, &error) != SQLITE_OK)
    {
        LoggerAPI::LogError(
            StringUtil::Format("WalletSqlite::GetNextChildPath - Failed to update account for user: %s. Error: %s",
...
std::unique_ptr<SlateContext> WalletSqlite::LoadSlateContext(const std::string &username,
                                                             ...
                                                             const uuids::uuid &slateId) const
{
    ...
    const std::string query =
        "SELECT enc_blind, enc_nonce FROM slate_contexts WHERE slate_id='" + uuids::to_string(slateId) + "'";
    if (sqlite3_prepare_v2(m_userDBs.at(username), query.c_str(), -1, &stmt, NULL) != SQLITE_OK)
    {
        LoggerAPI::LogError(StringUtil::Format("WalletSqlite::LoadSlateContext - Error while compiling sql: %s",
                                               sqlite3_errmsg(pDatabase)));
        sqlite3_finalize(stmt);
        throw WALLET_STORE_EXCEPTION("Error compiling statement.");
    }
    ...
    if (sqlite3_finalize(stmt) != SQLITE_OK)
    {
        LoggerAPI::LogError(StringUtil::Format("WalletSqlite::LoadSlateContext - Error finalizing statement: %s",
                                               sqlite3_errmsg(pDatabase)));
        throw WALLET_STORE_EXCEPTION("Error finalizing statement.");
    }
...
bool WalletSqlite::SaveSlateContext(const std::string &username, const SecureVector &masterSeed,
                                    const uuids::uuid &slateId, const SlateContext &slateContext)
{
    ...
    {
        LoggerAPI::LogError(StringUtil::Format("WalletSqlite::SaveSlateContext - Error while compiling sql: %s",
                                               sqlite3_errmsg(pDatabase)));
        sqlite3_finalize(stmt);
        throw WALLET_STORE_EXCEPTION("Error compiling statement.");
    }
    ...
    CBigInteger<32> encryptedBlindingFactor =
        slateContext.GetSecretKey().GetBytes() ^ SlateContext::DeriveXORKey(masterSeed, slateId, "blind");
    sqlite3_bind_blob(stmt, 2, (const void *)encryptedBlindingFactor.data(), 32, NULL);

    CBigInteger<32> encryptedNonce =
        slateContext.GetSecretNonce().GetBytes() ^ SlateContext::DeriveXORKey(masterSeed, slateId, "nonce");
    ...
    if (sqlite3_finalize(stmt) != SQLITE_OK)
    {
        LoggerAPI::LogError(StringUtil::Format("WalletSqlite::SaveSlateContext - Error finalizing statement: %s",
                                               sqlite3_errmsg(pDatabase)));
        throw WALLET_STORE_EXCEPTION("Error finalizing statement.");
    }
...
bool WalletSqlite::AddOutputs(const std::string &username, const SecureVector &masterSeed,
...
bool WalletSqlite::UpdateRefreshBlockHeight(const std::string &username, const uint64_t refreshBlockHeight)
{
    ...
    return SaveMetadata(username,
                        UserMetadata(metadata.GetNextTxId(), refreshBlockHeight, metadata.GetRestoreLeafIndex()));
}
...
bool WalletSqlite::UpdateRestoreLeafIndex(const std::string &username, const uint64_t lastLeafIndex)
{
    ...
    return SaveMetadata(username,
                        UserMetadata(metadata.GetNextTxId(), metadata.GetRefreshBlockHeight(), lastLeafIndex));
}
...
sqlite3 *WalletSqlite::CreateWalletDB(const std::string &username)
{
    ...
    {
        LoggerAPI::LogError("WalletSqlite::CreateWallet - Failed to create wallet.db for user: " + username +
                            ". Error: " + std::string(sqlite3_errmsg(pDatabase)));
        sqlite3_close(pDatabase);
    ...
    try
    {
        ...
        std::string tableCreation = "create table accounts(parent_path TEXT PRIMARY KEY, account_name TEXT NOT NULL, "
                                    "next_child_index INTEGER NOT NULL);";
        tableCreation +=
            "create table slate_contexts(slate_id TEXT PRIMARY KEY, enc_blind BLOB NOT NULL, enc_nonce BLOB NOT NULL);";
        // TODO: Add indices

        KeyChainPath nextChildPath = KeyChainPath::FromString("m/0/0").GetRandomChild();
        tableCreation += "insert into accounts values('m/0/0','DEFAULT'," +
                         std::to_string(nextChildPath.GetKeyIndices().back()) + ");";
