...
{
  public:
    PublicExtKey(const uint32_t network, const uint8_t depth, const uint32_t parentFingerprint,
    ...
    inline const PublicKey &GetPublicKey() const
    {
        return GetKeyBytes();
    }
    ...
    static PublicExtKey Deserialize(ByteBuffer &byteBuffer)
    {
        ...
        return PublicExtKey(network, depth, parentFingerprint, childNumber, std::move(chainCode),
                            std::move(compressedPublicKey));
    }
};