...
TEST_CASE("Reorg Chain")
{
	...
	TxBuilder::Criteria criteria;
	criteria.inputs = { input };
	criteria.outputs = { newOutput, changeOutput };
	criteria.include_change = false;
	criteria.fee_base = 0;
	criteria.num_kernels = 1;
	Transaction spendTransaction = txBuilder.BuildTx(criteria);
