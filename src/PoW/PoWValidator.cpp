...
PoWValidator::PoWValidator(const Config &config, const IBlockDB &blockDB) : m_config(config), m_blockDB(blockDB)
{
}
...
bool PoWValidator::IsPoWValid(const BlockHeader &header, const BlockHeader &previousHeader) const
{
    ...
    // Explicit check to ensure total_difficulty has increased by exactly the _network_ difficulty of the previous
    // block.
...
uint64_t PoWValidator::GetMaximumDifficulty(const BlockHeader &header) const
{
    ...
    // 2 proof of works, Cuckoo29 (for now) and Cuckoo30+, which are scaled differently (scaling not controlled for
    // now).
    ...
    {
        scalingDifficulty = (((uint64_t)2) << ((uint64_t)proofOfWork.GetEdgeBits() - Consensus::BASE_EDGE_BITS)) *
                            ((uint64_t)proofOfWork.GetEdgeBits());
    }

    std::vector<unsigned char> temp;
    temp.resize(sizeof(uint64_t));
    std::reverse_copy(proofOfWork.GetHash().GetData().cbegin(),
                      proofOfWork.GetHash().GetData().cbegin() + sizeof(uint64_t), temp.begin());
