...
{
  public:
    SlateVersionInfo(const uint16_t version) : m_version(version), m_originalVersion(version), m_blockHeaderVersion(1)
    {
    }
    ...
    inline const uint16_t GetVersion() const
    {
        return m_version;
    }
    inline const uint16_t GetOriginalVersion() const
    {
        return m_originalVersion;
    }
    inline const uint16_t GetBlockHeaderVersion() const
    {
    ...
    static SlateVersionInfo FromJSON(const Json::Value &versionInfoJSON)
    {
        ...
        const uint16_t blockHeaderVersion =
            (uint16_t)JsonUtil::GetRequiredField(versionInfoJSON, "block_header_version").asUInt();
