...
class GetBlockMessage : public IMessage
{
  public:
    ...
    // Constructors
    //
    GetBlockMessage(const Hash &hash) : m_hash(hash)
    {
    }
    GetBlockMessage(Hash &&hash) : m_hash(std::move(hash))
    {
    }
    ...
    // Clone
    //
    virtual GetBlockMessage *Clone() const override final
    {
        return new GetBlockMessage(*this);
    }
