...
{
  public:
    PublicKey(CBigInteger<33> &&compressedKey) : m_compressedKey(std::move(compressedKey))
    {
    }

    inline const CBigInteger<33> &GetCompressedBytes() const
    {
        return m_compressedKey;
    }

    inline const unsigned char *data() const
    {
        return m_compressedKey.data();
    }
    inline size_t size() const
    {
        return m_compressedKey.size();
    }
