...
std::optional<TorAddress> ForeignController::StartListener(const std::string &username, const SessionToken &token,
                                                           const SecureVector &seed)
{
    ...
    const char *pOptions[] = {"num_threads", "1", "listening_ports", "0", NULL};
    ...
    if (pTorKey != nullptr)
    {
        ...
        std::unique_ptr<TorAddress> pTorAddress =
            TorManager::GetInstance(m_config.GetTorConfig()).AddListener(hashedTorKey, portNumber);
...
int ForeignController::ForeignAPIHandler(mg_connection *pConnection, void *pCbContext)
{
    ...
    try
    {
        ...
        if (method == "receive_tx")
        {
            ...
            if (paramsOpt.has_value() && paramsOpt.value().isArray() && paramsOpt.value().size() == 3)
            {
                try
                {
                    ...
                    std::optional<std::string> accountName =
                        params[1].isNull() ? std::nullopt : std::make_optional<std::string>(params[1].asCString());
                    std::optional<std::string> message =
                        params[2].isNull() ? std::nullopt : std::make_optional<std::string>(params[2].asCString());

                    Context *pContext = (Context *)pCbContext;
                    std::unique_ptr<Slate> pReceivedSlate =
                        pContext->m_walletManager.Receive(pContext->m_token, slate, std::nullopt, message);
                    ...
                    {
                        responseJSON = RPC::Response::BuildError(request.GetId(), RPC::ErrorCode::INTERNAL_ERROR,
                                                                 "Failed to receive", std::nullopt)
                                           .ToJSON();
                    }
                }
                catch (DeserializationException &e)
                {
                    responseJSON = RPC::Response::BuildError(request.GetId(), RPC::ErrorCode::INVALID_PARAMS,
                                                             "Failed to deserialize slate", std::nullopt)
                                       .ToJSON();
                }
                catch (std::exception &e)
                {
                    responseJSON = RPC::Response::BuildError(request.GetId(), RPC::ErrorCode::INTERNAL_ERROR, e.what(),
                                                             std::nullopt)
                                       .ToJSON();
                }
            }
            ...
            {
                responseJSON = RPC::Response::BuildError(request.GetId(), RPC::ErrorCode::INVALID_PARAMS,
                                                         "Expected 3 parameters", std::nullopt)
                                   .ToJSON();
            }
        }
