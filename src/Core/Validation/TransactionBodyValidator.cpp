...
bool TransactionBodyValidator::ValidateWeight(const TransactionBody &transactionBody, const bool withReward)
{
    ...
    const uint32_t blockOutputWeight =
        (((uint32_t)transactionBody.GetOutputs().size() + reserve) * Consensus::BLOCK_OUTPUT_WEIGHT);
    const uint32_t blockKernelWeight =
        (((uint32_t)transactionBody.GetKernels().size() + reserve) * Consensus::BLOCK_KERNEL_WEIGHT);
...
bool TransactionBodyValidator::VerifyRangeProofs(const std::vector<TransactionOutput> &outputs)
{
    ...
    {
        rangeProofs.emplace_back(std::make_pair<Commitment, RangeProof>(Commitment(output.GetCommitment()),
