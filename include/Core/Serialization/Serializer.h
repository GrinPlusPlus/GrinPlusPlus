...
class Serializer
{
  public:
    ...
    template <class T> void Append(const T &t)
    {
    ...
    template <class T> void AppendLittleEndian(const T &t)
    {
        std::vector<unsigned char> temp(sizeof(T));
        memcpy(&temp[0], &t, sizeof(T));
    ...
    template <size_t NUM_BYTES> void AppendBigInteger(const CBigInteger<NUM_BYTES> &bigInteger)
    {
