...
Seeder::Seeder(const Config &config, ConnectionManager &connectionManager, PeerManager &peerManager,
...
//
// Continuously checks the number of connected peers, and connects to additional peers when the number of connections
// drops below the minimum. This function operates in its own thread.
//
            ...
            if (seeder.m_connectionManager.GetNumberOfActiveConnections() < maximumConnections)
            {
                ...
                if (connectionAdded)
                {
                    Connection *pConnection =
                        new Connection(std::move(socket), seeder.m_nextId++, seeder.m_config,
                                       seeder.m_connectionManager, seeder.m_peerManager, seeder.m_blockChainServer,
...
Connection *Seeder::SeedNewConnection()
{
    ...
    if (pPeer != nullptr)
    {
        ...
        Connection *pConnection =
            new Connection(Socket(SocketAddress(pPeer->GetIPAddress(), m_config.GetEnvironment().GetP2PPort())),
                           m_nextId++, m_config, m_connectionManager, m_peerManager, m_blockChainServer, connectedPeer);
        pConnection->Connect();
