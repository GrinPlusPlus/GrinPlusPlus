...
// Original reference edge_bits to compute difficulty factors for higher Cuckoo graph sizes, changing this would hard
// fork
...
static const uint64_t UNIT_DIFFICULTY =
    (((uint64_t)2) << (SECOND_POW_EDGE_BITS - BASE_EDGE_BITS)) * ((uint64_t)SECOND_POW_EDGE_BITS);
