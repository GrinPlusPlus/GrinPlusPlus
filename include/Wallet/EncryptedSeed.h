  ...
  public:
    EncryptedSeed(CBigInteger<16> &&iv, CBigInteger<8> &&salt, std::vector<unsigned char> &&encryptedSeedBytes,
        ...
        : m_iv(std::move(iv)), m_salt(std::move(salt)), m_encryptedSeedBytes(std::move(encryptedSeedBytes)),
          m_scryptParameters(std::move(scryptParameters))
    {
    }

    inline const CBigInteger<16> &GetIV() const
    {
        return m_iv;
    }
    inline const CBigInteger<8> &GetSalt() const
    {
        return m_salt;
    }
    inline const std::vector<unsigned char> &GetEncryptedSeedBytes() const
    {
        return m_encryptedSeedBytes;
    }
    inline const ScryptParameters &GetScryptParameters() const
    {
        return m_scryptParameters;
    }
    ...
    static EncryptedSeed FromJSON(const Json::Value &json)
    {
        ...
        std::vector<unsigned char> encryptedSeedBytes =
            HexUtil::FromHex(JsonUtil::GetRequiredField(json, "seed").asString());
