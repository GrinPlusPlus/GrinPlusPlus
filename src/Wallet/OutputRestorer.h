...
class OutputRestorer
{
  public:
    ...
    std::vector<OutputData> FindAndRewindOutputs(const SecureVector &masterSeed, Wallet &wallet,
                                                 const bool fromGenesis) const;

  private:
    std::unique_ptr<OutputData> GetWalletOutput(const SecureVector &masterSeed,
