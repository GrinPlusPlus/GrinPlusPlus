...
{
    std::string tableCreation = "create table metadata(id INTEGER PRIMARY KEY, next_tx_id INTEGER NOT NULL, "
                                "refresh_block_height INTEGER NOT NULL, restore_leaf_index INTEGER NOT NULL);";
    ...
    {
        LoggerAPI::LogError(
            StringUtil::Format("MetadataTable::CreateTable - Create metadata table failed with error: %s", error));
        sqlite3_free(error);
...
void MetadataTable::UpdateSchema(sqlite3 &database, const int previousVersion)
{
    if (previousVersion == 0)
    {
        ...
        UserMetadata newMetadata(metadata.GetNextTxId(), metadata.GetRefreshBlockHeight(), 0);
...
UserMetadata MetadataTable::GetMetadata(sqlite3 &database)
{
    ...
    if (sqlite3_prepare_v2(&database,
                           "SELECT next_tx_id, refresh_block_height, restore_leaf_index FROM metadata WHERE ID=1", -1,
                           &stmt, NULL) != SQLITE_OK)
    {
        LoggerAPI::LogError(StringUtil::Format("MetadataTable::GetMetadata - Error while compiling sql: %s",
    ...
    {
        LoggerAPI::LogError(StringUtil::Format("MetadataTable::GetMetadata - Error while performing sql: %s",
                                               sqlite3_errmsg(&database)));
    ...
    {
        LoggerAPI::LogError(StringUtil::Format("MetadataTable::GetMetadata - Error finalizing statement: %s",
                                               sqlite3_errmsg(&database)));
...
void MetadataTable::SaveMetadata(sqlite3 &database, const UserMetadata &userMetadata)
{
    std::string update = StringUtil::Format(
        ...
        userMetadata.GetNextTxId(), userMetadata.GetRefreshBlockHeight(), userMetadata.GetRestoreLeafIndex());

    char *error = nullptr;
    if (sqlite3_exec(&database, update.c_str(), NULL, NULL, &error) != SQLITE_OK)
    {
        LoggerAPI::LogError(
            StringUtil::Format("MetadataTable::SaveMetadata - Failed to save metadata for user. Error: %s", error));
        sqlite3_free(error);
