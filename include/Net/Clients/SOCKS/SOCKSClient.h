...
{
  public:
    SOCKSClient(SocketAddress &&proxyAddress) : m_proxyAddress(std::move(proxyAddress))
    {
    }
  ...
  private:
    ...
    void Initialize(const SOCKS::ProxyCredentials *auth)
    {
        ...
        {
            data =
                std::vector<uint8_t>({SOCKS::Version::SOCKS5, 0x02, SOCKS::Method::NOAUTH, SOCKS::Method::USER_PASS});
        }
        else
        {
    ...
    SOCKS::Destination ReadDestination(const SOCKS::Atyp &type)
    {
        ...
        case SOCKS::Atyp::IPV4: {
            ...
            destination.ipAddress =
                std::make_optional<IPAddress>(IPAddress::FromIP(address[0], address[1], address[2], address[3]));
            break;
        }
