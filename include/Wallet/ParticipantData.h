...
class ParticipantData
{
  public:
    ...
    ParticipantData(const uint64_t participantId, PublicKey &&publicBlindExcess, PublicKey &&publicNonce,
                    std::optional<CompactSignature> &&partialSigOpt, std::optional<std::string> &&messageOpt,
                    std::optional<CompactSignature> &&messageSigOpt)
        : m_participantId(participantId), m_publicBlindExcess(publicBlindExcess), m_publicNonce(publicNonce),
          m_partialSignatureOpt(std::move(partialSigOpt)), m_messageOpt(std::move(messageOpt)),
          m_messageSignatureOpt(std::move(messageSigOpt))
    {
    }

    inline uint64_t GetParticipantId() const
    {
        return m_participantId;
    }
    inline const PublicKey &GetPublicBlindExcess() const
    {
        return m_publicBlindExcess;
    }
    inline const PublicKey &GetPublicNonce() const
    {
        return m_publicNonce;
    }
    inline const std::optional<CompactSignature> &GetPartialSignature() const
    {
        return m_partialSignatureOpt;
    }
    inline const std::optional<std::string> &GetMessageText() const
    {
        return m_messageOpt;
    }
    inline const std::optional<CompactSignature> &GetMessageSignature() const
    {
        return m_messageSignatureOpt;
    }

    inline void AddPartialSignature(const CompactSignature &signature)
    {
        m_partialSignatureOpt = std::make_optional<CompactSignature>(signature);
    }
    ...
    inline Json::Value ToJSON(const bool hex) const
    {
        ...
        participantJSON["public_blind_excess"] =
            JsonUtil::ConvertToJSON(GetPublicBlindExcess().GetCompressedBytes().GetData(), hex);
    ...
    static ParticipantData FromJSON(const Json::Value &participantJSON, const bool hex)
    {
        ...
        return ParticipantData(participantId, std::move(publicBlindExcess), std::move(publicNonce),
                               std::move(partialSigOpt), std::move(messageOpt), std::move(messageSigOpt));
    }
