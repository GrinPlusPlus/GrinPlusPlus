	...
	const std::string& username)
{
    ...
    int currentAddressIndex = walletDB.Read()->GetCurrentAddressIndex(userPath);
	KeyChainPath currentAddressPath = KeyChainPath::FromString("m/0/1").GetChild(currentAddressIndex);
	ed25519_keypair_t torKey = KeyChain::FromSeed(masterSeed).DeriveED25519Key(currentAddressPath);
