...
{
  public:
    KeyChainPath(std::vector<uint32_t> &&keyIndices) : m_keyIndices(keyIndices)
    {
    }
    ...
    // Operators
    //
    inline bool operator<(const KeyChainPath &other) const
    {
        return GetKeyIndices() < other.GetKeyIndices();
    }
    inline bool operator==(const KeyChainPath &other) const
    {
        return GetKeyIndices() == other.GetKeyIndices();
    ...
    const std::vector<uint32_t> &GetKeyIndices() const
    {
        return m_keyIndices;
    }
