...
class ShakeMessage : public IMessage
{
  public:
    ...
    // Constructors
    //
    ShakeMessage(const uint32_t version, const Capabilities &capabilities, CBigInteger<32> &&hash,
                 const uint64_t totalDifficulty, const std::string &userAgent)
        : m_version(version), m_capabilities(capabilities), m_hash(std::move(hash)), m_totalDifficulty(totalDifficulty),
    ...
    // Clone
    //
    virtual ShakeMessage *Clone() const override final
    {
        return new ShakeMessage(*this);
    }
