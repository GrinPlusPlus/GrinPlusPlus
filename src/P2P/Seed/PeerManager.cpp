...
PeerManager::PeerManager(const Config &config, IPeerDB &peerDB) : m_config(config), m_peerDB(peerDB)
{
}
...
bool PeerManager::ArePeersNeeded(const Capabilities::ECapability &preferredCapability) const
{
    ...
    const time_t maxBanTime =
        std::chrono::system_clock::to_time_t(std::chrono::system_clock::now() - std::chrono::seconds(P2P::BAN_WINDOW));
...
std::vector<Peer> PeerManager::GetPeers(const Capabilities::ECapability &preferredCapability,
...
std::vector<Peer> PeerManager::GetPeersWithCapability(const Capabilities::ECapability &preferredCapability,
                                                      const uint16_t maxPeers, const bool connectingToPeer) const
{
    ...
    for (auto iter = m_peersByAddress.begin(); iter != m_peersByAddress.end(); iter++)
    {
        ...
        {
            if (!connectingToPeer ||
                (!peerEntry.m_connected && std::difftime(currentTime, peerEntry.m_lastAttempt) > P2P::RETRY_WINDOW))
            {
                if (connectingToPeer)
                {
