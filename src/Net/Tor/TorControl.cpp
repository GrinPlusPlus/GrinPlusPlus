...
TorControl::TorControl(const TorConfig &config) : m_torConfig(config)
{
}
...
bool TorControl::Initialize()
{
    ...
    const std::string command = StringUtil::Format(
        ...
        m_torConfig.GetControlPort(), m_torConfig.GetSocksPort(), m_torConfig.GetHashedControlPassword().c_str());
    ...
    if (m_processId > 0)
    {
        // Open control socket
        bool connected =
            m_client.Connect(SocketAddress(IPAddress::FromString("127.0.0.1"), m_torConfig.GetControlPort()));
