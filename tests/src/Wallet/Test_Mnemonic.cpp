...
{
	{
		...
		for (const Json::Value& vector : vectors)
		{
			SecureString walletWords = vector.get(Json::ArrayIndex(1), Json::nullValue).asString().c_str();
			std::optional<SecureVector> entropyOpt = Mnemonic().ToEntropy(walletWords);

			std::string hex = vector.get(Json::ArrayIndex(0), Json::nullValue).asString();
			...
			if (data.size() == 16)
			{
				expected = CBigInteger<16>::FromHex(hex).GetData();
			}
			else if (data.size() == 24)
			{
				expected = CBigInteger<24>::FromHex(hex).GetData();
			}
			else {
				expected = CBigInteger<32>::FromHex(hex).GetData();
			}
