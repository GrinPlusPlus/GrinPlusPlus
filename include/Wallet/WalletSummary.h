...
{
  public:
    WalletSummary(const uint64_t lastConfirmedHeight, const uint64_t minimumConfirmations, const uint64_t total,
                  const uint64_t unconfirmed, const uint64_t immature, const uint64_t locked, const uint64_t spendable,
                  std::vector<WalletTx> &&transactions)
        : m_lastConfirmedHeight(lastConfirmedHeight), m_minimumConfirmations(minimumConfirmations), m_total(total),
          m_amountAwaitingConfirmation(unconfirmed), m_amountImmature(immature), m_amountLocked(locked),
          m_amountCurrentlySpendable(spendable), m_transactions(std::move(transactions))
    {
    }

    inline uint64_t GetLastConfirmedHeight() const
    {
        return m_lastConfirmedHeight;
    }
    inline uint64_t GetMinimumConfirmations() const
    {
    ...
    inline uint64_t GetAmountAwaitingConfirmation() const
    {
        return m_amountAwaitingConfirmation;
    }
    inline uint64_t GetAmountImmature() const
    {
        return m_amountImmature;
    }
    inline uint64_t GetAmountLocked() const
    {
        return m_amountLocked;
    }
    inline uint64_t GetAmountCurrentlySpendable() const
    {
        return m_amountCurrentlySpendable;
    }
    inline const std::vector<WalletTx> &GetTransactions() const
    {
