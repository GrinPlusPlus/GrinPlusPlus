...
TransactionKernel::TransactionKernel(const EKernelFeatures features, const uint64_t fee, const uint64_t lockHeight,
                                     Commitment &&excessCommitment, Signature &&excessSignature)
    : m_features(features), m_fee(fee), m_lockHeight(lockHeight), m_excessCommitment(std::move(excessCommitment)),
...
TransactionKernel TransactionKernel::Deserialize(ByteBuffer &byteBuffer)
{
    ...
    return TransactionKernel((EKernelFeatures)features, fee, lockHeight, std::move(excessCommitment),
...
TransactionKernel TransactionKernel::FromJSON(const Json::Value &transactionKernelJSON, const bool hex)
{
    const EKernelFeatures features =
        KernelFeatures::FromString(JsonUtil::GetRequiredField(transactionKernelJSON, "features").asString());
