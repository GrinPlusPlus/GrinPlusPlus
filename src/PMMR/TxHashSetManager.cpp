...
ITxHashSet *TxHashSetManager::LoadFromZip(const Config &config, IBlockDB &blockDB, const std::string &zipFilePath,
...
bool TxHashSetManager::SaveSnapshot(const BlockHeader &blockHeader, const std::string &zipFilePath)
{
...

    {
        ...
        FileUtil::RenameFile(snapshotDir + "output/pmmr_leaf.bin",
                             snapshotDir + "output/pmmr_leaf.bin." + blockHeader.FormatHash());
        FileUtil::RenameFile(snapshotDir + "rangeproof/pmmr_leaf.bin",
                             snapshotDir + "rangeproof/pmmr_leaf.bin." + blockHeader.FormatHash());

        // 8. Create Zip
        const std::vector<std::string> pathsToZip = {snapshotDir + "kernel", snapshotDir + "output",
