...
ProofOfWork::ProofOfWork(const uint8_t edgeBits, std::vector<uint64_t> &&proofNonces)
    : m_edgeBits(edgeBits), m_proofNonces(std::move(proofNonces))
{
}

ProofOfWork::ProofOfWork(const uint8_t edgeBits, std::vector<uint64_t> &&proofNonces, Hash &&hash)
    : m_edgeBits(edgeBits), m_proofNonces(std::move(proofNonces)), m_hash(hash)
{
}
...
std::vector<uint64_t> ProofOfWork::DeserializeProofNonces(const std::vector<unsigned char> &bits,
