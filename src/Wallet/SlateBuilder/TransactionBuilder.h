...
{
  public:
    static Transaction BuildTransaction(const std::vector<OutputData> &inputs,
                                        const std::vector<OutputData> &changeOutputs,
                                        const BlindingFactor &transactionOffset, const uint64_t fee,
                                        const uint64_t lockHeight)
    {
        auto getInput = [](OutputData &input) -> TransactionInput {
            return TransactionInput(input.GetOutput().GetFeatures(), Commitment(input.GetOutput().GetCommitment()));
        };
        std::vector<TransactionInput> transactionInputs =
            FunctionalUtil::map<std::vector<TransactionInput>>(inputs, getInput);
        ...
        std::vector<TransactionOutput> transactionOutputs =
            FunctionalUtil::map<std::vector<TransactionOutput>>(changeOutputs, getOutput);

        const EKernelFeatures kernelFeatures =
            (lockHeight == 0) ? EKernelFeatures::DEFAULT_KERNEL : EKernelFeatures::HEIGHT_LOCKED;
        TransactionKernel kernel(kernelFeatures, fee, lockHeight, Commitment(CBigInteger<33>::ValueOf(0)),
                                 Signature(CBigInteger<64>::ValueOf(0)));
        ...
        return Transaction(
            BlindingFactor(transactionOffset),
            TransactionBody(std::move(transactionInputs), std::move(transactionOutputs), std::move(kernels)));
    }
