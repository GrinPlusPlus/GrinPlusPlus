...
{
  public:
    ByteBuffer(const std::vector<unsigned char> &bytes) : m_index(0), m_bytes(bytes)
    {
    }

    template <class T> void ReadBigEndian(T &t)
    {
    ...
    template <class T> void ReadLittleEndian(T &t)
    {
        if (m_index + sizeof(T) > m_bytes.size())
        {
            throw DeserializationException();
    ...
    template <size_t NUM_BYTES> CBigInteger<NUM_BYTES> ReadBigInteger()
    {
