...
class WalletRocksDB : public IWalletDB
{
  public:
    ...
    virtual std::unique_ptr<SlateContext> LoadSlateContext(const std::string &username, const SecureVector &masterSeed,
                                                           const uuids::uuid &slateId) const override final;
    virtual bool SaveSlateContext(const std::string &username, const SecureVector &masterSeed,
                                  const uuids::uuid &slateId, const SlateContext &slateContext) override final;

    virtual bool AddOutputs(const std::string &username, const SecureVector &masterSeed,
                            const std::vector<OutputData> &outputs) override final;
    virtual std::vector<OutputData> GetOutputs(const std::string &username,
                                               const SecureVector &masterSeed) const override final;

    virtual bool AddTransaction(const std::string &username, const SecureVector &masterSeed,
                                const WalletTx &walletTx) override final;
    virtual std::vector<WalletTx> GetTransactions(const std::string &username,
                                                  const SecureVector &masterSeed) const override final;
    ...
    virtual bool UpdateRefreshBlockHeight(const std::string &username,
                                          const uint64_t refreshBlockHeight) override final;
  ...
  private:
    ...
    static std::vector<unsigned char> Encrypt(const SecureVector &masterSeed, const std::string &dataType,
                                              const SecureVector &bytes);
    static SecureVector Decrypt(const SecureVector &masterSeed, const std::string &dataType,
