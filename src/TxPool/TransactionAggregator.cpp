...
std::unique_ptr<Transaction> TransactionAggregator::Aggregate(const std::vector<Transaction> &transactions)
{
    ...
    const std::unique_ptr<BlindingFactor> pTotalKernelOffset =
        Crypto::AddBlindingFactors(kernelOffsets, std::vector<BlindingFactor>());
    ...
    return std::make_unique<Transaction>(
        Transaction(BlindingFactor(*pTotalKernelOffset),
