...
// TODO: Rewrite this
// 0. Initial login after upgrade - for every output, find matching WalletTx and update OutputData TxId. If none found,
// create new WalletTx.
//
// 1. Check for own outputs in new blocks.
// 2. For each output, look for OutputData with matching commitment. If no output found, create new WalletTx and
// OutputData.
...
std::vector<OutputData> WalletRefresher::Refresh(const SecureVector &masterSeed, Wallet &wallet, const bool fromGenesis)
{
    ...
    std::vector<OutputData> restoredOutputs =
        OutputRestorer(m_config, m_nodeClient, keyChain).FindAndRewindOutputs(masterSeed, wallet, fromGenesis);

    // 2. For each restored output, look for OutputData with matching commitment.
    for (OutputData &restoredOutput : restoredOutputs)
    {
        LoggerAPI::LogInfo("WalletRefresher::Refresh - Output found at index " +
                           std::to_string(restoredOutput.GetMMRIndex().value_or(0)));
        ...
        if (restoredOutput.GetStatus() != EOutputStatus::SPENT)
        {
            ...
            if (pExistingOutput == nullptr)
            {
                ...
                WalletTx walletTx(walletTxId, EWalletTxType::RECEIVED, std::nullopt, std::nullopt, std::nullopt,
                                  std::chrono::system_clock::now(),
                                  std::nullopt, // TODO: Lookup block time
                                  restoredOutput.GetBlockHeight(), restoredOutput.GetAmount(), 0, std::nullopt,
                                  std::nullopt);
            ...
            // else if (pExistingOutput->GetStatus() == EOutputStatus::SPENT || pExistingOutput->GetStatus() ==
            // EOutputStatus::CANCELED)
            //{
...
void WalletRefresher::RefreshOutputs(const SecureVector &masterSeed, Wallet &wallet,
                                     std::vector<OutputData> &walletOutputs)
{
    ...
    for (const OutputData &outputData : walletOutputs)
    {
        ...
        {
            LoggerAPI::LogTrace("WalletRefresher::RefreshOutputs - No need to refresh spent/canceled output with
        commitment: " + commitment.ToHex()); continue;
        }*/

        // TODO: What if commitment has mmr_index?
        LoggerAPI::LogDebug("WalletRefresher::RefreshOutputs - Refreshing output with commitment: " +
                            commitment.ToHex());
    ...
    for (OutputData &outputData : walletOutputs)
    {
        ...
        if (iter != outputLocations.cend())
        {
            if (outputData.GetStatus() != EOutputStatus::LOCKED)
            {
                ...
                if (WalletUtil::IsOutputImmature(features, outputBlockHeight, lastConfirmedHeight,
                                                 minimumConfirmations))
                {
                    if (outputData.GetStatus() != EOutputStatus::IMMATURE)
                    {
                        LoggerAPI::LogDebug("WalletRefresher::RefreshOutputs - Marking output as immature: " +
                                            outputData.GetOutput().GetCommitment().ToHex());
                ...
                {
                    LoggerAPI::LogDebug("WalletRefresher::RefreshOutputs - Marking output as spendable: " +
                                        outputData.GetOutput().GetCommitment().ToHex());
                 ...
                 outputData.GetStatus() != EOutputStatus::SPENT && outputData.GetStatus() != EOutputStatus::CANCELED)
        {
            // TODO: Check if (lastConfirmedHeight > outputData.GetConfirmedHeight)
            LoggerAPI::LogDebug("WalletRefresher::RefreshOutputs - Marking output as spent: " +
                                outputData.GetOutput().GetCommitment().ToHex());
...
void WalletRefresher::RefreshTransactions(const std::string &username, const SecureVector &masterSeed,
                                          const std::vector<OutputData> &refreshedOutputs,
        ...
        if (output.GetWalletTxId().has_value())
        {
            ...
            if (iter != walletTransactionsById.cend())
            {
                ...
                if (output.GetBlockHeight().has_value())
                {
                    ...
                    {
                        LoggerAPI::LogDebug("WalletRefresher::RefreshTransactions - Marking transaction as received: " +
                    ...
                    {
                        LoggerAPI::LogDebug("WalletRefresher::RefreshTransactions - Marking transaction as sent: " +
                                            std::to_string(walletTx.GetId()));
        ...
        if (walletTx.GetTransaction().has_value() && walletTx.GetType() == EWalletTxType::SENDING_FINALIZED)
        {
            ...
            for (const TransactionOutput &output : outputs)
            {
                ...
                if (pOutputData != nullptr)
                {
                    ...
                    {
                        LoggerAPI::LogDebug(
                            "WalletRefresher::RefreshTransactions - Output is spent. Marking transaction as sent: " +
                            walletTx.GetId());
...
std::unique_ptr<OutputData> WalletRefresher::FindOutput(const std::vector<OutputData> &walletOutputs,
