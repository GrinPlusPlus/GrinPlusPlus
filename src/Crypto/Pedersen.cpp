...
std::unique_ptr<Commitment> Pedersen::PedersenCommit(const uint64_t value, const BlindingFactor &blindingFactor) const
{
    ...
    const int result = secp256k1_pedersen_commit(m_pContext, &commitment, &blindingFactor.GetBytes()[0], value,
    ...
    LOG_ERROR_F("Failed to create commitment. Result: %d, Value: %llu, Blind: %s", result, value,
                blindingFactor.GetBytes().ToHex().c_str());
    return std::unique_ptr<Commitment>(nullptr);
}

std::unique_ptr<Commitment> Pedersen::PedersenCommitSum(const std::vector<Commitment> &positive,
                                                        const std::vector<Commitment> &negative) const
{
    std::shared_lock<std::shared_mutex> readLock(m_mutex);

    std::vector<secp256k1_pedersen_commitment *> positiveCommitments =
        Pedersen::ConvertCommitments(*m_pContext, positive);
    std::vector<secp256k1_pedersen_commitment *> negativeCommitments =
        Pedersen::ConvertCommitments(*m_pContext, negative);

    secp256k1_pedersen_commitment commitment;
    const int result = secp256k1_pedersen_commit_sum(
        m_pContext, &commitment, positiveCommitments.empty() ? nullptr : &positiveCommitments[0],
        positiveCommitments.size(), negativeCommitments.empty() ? nullptr : &negativeCommitments[0],
...
std::unique_ptr<BlindingFactor> Pedersen::PedersenBlindSum(const std::vector<BlindingFactor> &positive,
                                                           const std::vector<BlindingFactor> &negative) const
{
    std::shared_lock<std::shared_mutex> readLock(m_mutex);
    ...
    const int result = secp256k1_pedersen_blind_sum(m_pContext, &blindingFactorBytes[0], blindingFactors.data(),
                                                    blindingFactors.size(), positive.size());
...
std::unique_ptr<SecretKey> Pedersen::BlindSwitch(const SecretKey &blindingFactor, const uint64_t amount) const
{
    ...
    const int result =
        secp256k1_blind_switch(m_pContext, blindSwitch.data(), blindingFactor.data(), amount,
                               &secp256k1_generator_const_h, &secp256k1_generator_const_g, &GENERATOR_J_PUB);
    if (result == 1)
    {
...
std::vector<secp256k1_pedersen_commitment *> Pedersen::ConvertCommitments(const secp256k1_context &context,
