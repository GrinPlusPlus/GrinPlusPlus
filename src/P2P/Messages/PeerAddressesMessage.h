...
class PeerAddressesMessage : public IMessage
{
  public:
    ...
    // Constructors
    //
    PeerAddressesMessage(std::vector<SocketAddress> &&peerAddresses) : m_peerAddresses(std::move(peerAddresses))
    {
    }
    ...
    // Clone
    //
    virtual PeerAddressesMessage *Clone() const override final
    {
        return new PeerAddressesMessage(*this);
    }
    ...
    {
        return MessageTypes::PeerAddrs;
    }
    inline const std::vector<SocketAddress> &GetPeerAddresses() const
    {
        return m_peerAddresses;
    }
