...
class BlockChainServer : public IBlockChainServer
{
  public:
    ...
    // If this is the first time opening BitcoinDB (ie. no blockchain database exists), the blockchain is populated with
    ...
    virtual EBlockChainStatus ProcessTransactionHashSet(const Hash &blockHash, const std::string &path,
                                                        SyncStatus &syncStatus) override final;
    ...
    virtual std::unique_ptr<BlockHeader> GetBlockHeaderByHeight(const uint64_t height,
                                                                const EChainType chainType) const override final;
    virtual std::unique_ptr<BlockHeader> GetBlockHeaderByHash(const CBigInteger<32> &hash) const override final;
    virtual std::unique_ptr<BlockHeader> GetBlockHeaderByCommitment(
        const Commitment &outputCommitment) const override final;
    virtual std::unique_ptr<BlockHeader> GetTipBlockHeader(const EChainType chainType) const override final;
    virtual std::vector<BlockHeader> GetBlockHeadersByHash(
        const std::vector<CBigInteger<32>> &hashes) const override final;
    ...
    virtual std::vector<BlockWithOutputs> GetOutputsByHeight(const uint64_t startHeight,
