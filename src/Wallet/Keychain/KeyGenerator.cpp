...
KeyGenerator::KeyGenerator(const Config &config) : m_config(config)
{
}
...
PrivateExtKey KeyGenerator::GenerateMasterKey(const SecureVector &seed, const EKeyChainType &keyChainType) const
{
    ...
    {
        return PrivateExtKey::Create(BitUtil::ConvertToU32(42, 1, 0, 42), 0, 0, 0, std::move(masterChainCode),
                                     std::move(masterSecretKey));
    }
    else
    {
        return PrivateExtKey::Create(m_config.GetWalletConfig().GetPrivateKeyVersion(), 0, 0, 0,
                                     std::move(masterChainCode), std::move(masterSecretKey));
    }
}

std::unique_ptr<PrivateExtKey> KeyGenerator::GenerateChildPrivateKey(const PrivateExtKey &parentExtendedKey,
                                                                     const uint32_t childKeyIndex) const
{
    ...
    CBigInteger<20> parentIdentifier =
        Crypto::RipeMD160(Crypto::SHA256(pPublicKey->GetPublicKey().GetCompressedBytes().GetData()).GetData());
    const uint32_t parentFingerprint =
        BitUtil::ConvertToU32(parentIdentifier[0], parentIdentifier[1], parentIdentifier[2], parentIdentifier[3]);

    Serializer serializer(
        37); // Reserve 37 bytes: 1 byte for 0x00 padding (hardened) or 0x02/0x03 point parity (normal), 32 bytes for
             // private key (hardened) or public key X coord, 4 bytes for index.
    ...
    const CBigInteger<64> hmacSha512 =
        Crypto::HMAC_SHA512(parentExtendedKey.GetChainCode().GetBytes().GetData(), serializer.GetBytes());
    ...
    return std::make_unique<PrivateExtKey>(
        PrivateExtKey::Create(m_config.GetWalletConfig().GetPrivateKeyVersion(), parentExtendedKey.GetDepth() + 1,
                              parentFingerprint, childKeyIndex, std::move(childChainCode), std::move(childPrivateKey)));
}

PublicExtKey KeyGenerator::GenerateChildPublicKey(const PublicExtKey &parentExtendedPublicKey,
                                                  const uint32_t childKeyIndex) const
{
    throw std::exception();
}
