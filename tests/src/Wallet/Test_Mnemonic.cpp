...
{
	{
		...
		REQUIRE(((const std::vector<unsigned char>&)*entropyOpt) == expected);
	}
...

	{
		...
		std::optional<SecureVector> entropyOpt = Mnemonic().ToEntropy(walletWords);
		REQUIRE(HexUtil::ConvertToHex((const std::vector<unsigned char>&)*entropyOpt) == "0a069abaeacc005d5f04a94885d3bdf9a2808c4169d9b374726f3093e70c6efd");
	}
}