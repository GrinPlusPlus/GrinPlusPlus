...
void BlockChainServer::Initialize()
{
    ...
    m_pChainState =
        new ChainState(m_config, *m_pChainStore, *m_pBlockStore, *m_pHeaderMMR, m_transactionPool, m_txHashSetManager);
...
EBlockChainStatus BlockChainServer::AddCompactBlock(const CompactBlock &compactBlock)
{
    ...
    std::unique_ptr<BlockHeader> pConfirmedHeader =
        m_pChainState->GetBlockHeaderByHeight(compactBlock.GetBlockHeader().GetHeight(), EChainType::CONFIRMED);
...
std::string BlockChainServer::SnapshotTxHashSet(const BlockHeader &blockHeader)
{
    ...
    const std::string destination =
        fs::temp_directory_path().string() + "Snapshots/TxHashSet." + blockHeader.FormatHash() + ".zip";
...
{
    const bool success = TxHashSetProcessor(m_config, *this, *m_pChainState, m_database.GetBlockDB())
                             .ProcessTxHashSet(blockHash, path, syncStatus);
...
std::unique_ptr<BlockHeader> BlockChainServer::GetBlockHeaderByHeight(const uint64_t height,
                                                                      const EChainType chainType) const
{
...
std::vector<BlockWithOutputs> BlockChainServer::GetOutputsByHeight(const uint64_t startHeight,
                                                                   const uint64_t maxHeight) const
{
...
{
    std::unique_ptr<BlockHeader> pNextHeader = m_pChainState->GetBlockHeaderByHeight(
        m_pChainState->GetHeight(EChainType::CONFIRMED) + 1, EChainType::CANDIDATE);
    if (pNextHeader == nullptr)
    {
        return false;
    }

    std::unique_ptr<FullBlock> pOrphanBlock =
        m_pChainState->GetOrphanBlock(pNextHeader->GetHeight(), pNextHeader->GetHash());
    if (pOrphanBlock == nullptr)
    {
        return false;
    }

    return BlockProcessor(m_config, m_pBlockStore->GetBlockDB(), *m_pChainState).ProcessBlock(*pOrphanBlock) ==
...
EXPORT IBlockChainServer *StartBlockChainServer(const Config &config, IDatabase &database,
                                                TxHashSetManager &txHashSetManager, ITransactionPool &transactionPool)
{
