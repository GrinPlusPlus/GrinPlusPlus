...
bool WalletRestServer::Initialize()
{
    ...
    const char *pOwnerOptions[] = {"num_threads", "1", "listening_ports", std::to_string(ownerPort).c_str(), NULL};

    m_pOwnerCivetContext = mg_start(NULL, 0, pOwnerOptions);
    mg_set_request_handler(m_pOwnerCivetContext, "/v1/wallet/owner/", WalletRestServer::OwnerAPIHandler,
                           m_pWalletContext);
...
int WalletRestServer::OwnerAPIHandler(mg_connection *pConnection, void *pWalletContext)
{
    ...
    try
    {
        ...
        {
            return OwnerPostAPI(pContext->m_config)
                .HandlePOST(pConnection, action, *pContext->m_pWalletManager, *pContext->m_pNodeClient);
        }
    }
