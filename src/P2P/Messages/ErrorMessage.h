...
class ErrorMessage : public IMessage
{
  public:
    ...
    // Constructors
    //
    ErrorMessage(const uint32_t errorCode, const std::string &message) : m_errorCode(errorCode), m_message(message)
    {
    }
    ...
    // Clone
    //
    virtual ErrorMessage *Clone() const override final
    {
    ...
    inline const uint32_t GetErrorCode() const
    {
        return m_errorCode;
    }
