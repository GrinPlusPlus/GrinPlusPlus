...
std::string ConfigReader::ReadDataPath(const Json::Value &root, const EEnvironmentType environmentType) const
{
    ...
    const std::string defaultPath = FileUtil::GetHomeDirectory() + "/.GrinPP/" + dataDirectory +
                                    "/"; //+ SEPARATOR + ".GrinPP" + SEPARATOR + dataDirectory  + SEPARATOR;
...
WalletConfig ConfigReader::ReadWalletConfig(const Json::Value &root, const EEnvironmentType environmentType,
                                            const std::string &dataPath) const
{
    ...
    return WalletConfig(walletPath, databaseType, listenPort, ownerPort, publicKeyVersion, privateKeyVersion,
