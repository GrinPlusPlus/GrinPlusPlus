...
class Commitment
{
  public:
    ...
    // Constructors
    //
    Commitment(CBigInteger<33> &&commitmentBytes) : m_commitmentBytes(std::move(commitmentBytes))
    {
    }
    Commitment(const CBigInteger<33> &commitmentBytes) : m_commitmentBytes(commitmentBytes)
    {
    }
    ...
    inline bool operator<(const Commitment &rhs) const
    {
        return m_commitmentBytes < rhs.GetCommitmentBytes();
    }
    inline bool operator!=(const Commitment &rhs) const
    {
        return m_commitmentBytes != rhs.GetCommitmentBytes();
    ...
    // Getters
    //
    inline const CBigInteger<33> &GetCommitmentBytes() const
    {
        return m_commitmentBytes;
    }
...
namespace std
{
template <> struct hash<Commitment>
{
    size_t operator()(const Commitment &commitment) const
    {
        ...
        return BitUtil::ConvertToU64(bytes[0], bytes[4], bytes[8], bytes[12], bytes[16], bytes[20], bytes[24],
                                     bytes[28]);
    }
};
} // namespace std