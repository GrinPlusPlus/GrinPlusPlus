...
BlockHeader::BlockHeader(const uint16_t version, const uint64_t height, const int64_t timestamp,
                         CBigInteger<32> &&previousBlockHash, CBigInteger<32> &&previousRoot,
                         CBigInteger<32> &&outputRoot, CBigInteger<32> &&rangeProofRoot, CBigInteger<32> &&kernelRoot,
                         BlindingFactor &&totalKernelOffset, const uint64_t outputMMRSize, const uint64_t kernelMMRSize,
                         const uint64_t totalDifficulty, const uint32_t scalingDifficulty, const uint64_t nonce,
                         ProofOfWork &&proofOfWork)
    : m_version(version), m_height(height), m_timestamp(timestamp), m_previousBlockHash(std::move(previousBlockHash)),
      m_previousRoot(std::move(previousRoot)), m_outputRoot(std::move(outputRoot)),
      m_rangeProofRoot(std::move(rangeProofRoot)), m_kernelRoot(std::move(kernelRoot)),
      m_totalKernelOffset(std::move(totalKernelOffset)), m_outputMMRSize(outputMMRSize), m_kernelMMRSize(kernelMMRSize),
      m_totalDifficulty(totalDifficulty), m_scalingDifficulty(scalingDifficulty), m_nonce(nonce),
...
BlockHeader BlockHeader::Deserialize(ByteBuffer &byteBuffer)
{
    ...
    return BlockHeader(version, height, timestamp, std::move(previousBlockHash), std::move(previousRoot),
                       std::move(outputRoot), std::move(rangeProofRoot), std::move(kernelRoot),
                       std::move(totalKernelOffset), outputMMRSize, kernelMMRSize, totalDifficulty, scalingDifficulty,
                       nonce, std::move(proofOfWork));
}
