...
template <class Request, class Response> class Client
{
  ...
  protected:
    ...
    std::string ReadLine(const asio::chrono::steady_clock::duration &timeout)
    {
        ...
        asio::async_read_until(
            m_socket, asio::dynamic_buffer(m_stringBuffer), '\n',
    ...
    std::vector<uint8_t> Read(const size_t numBytes, const asio::chrono::steady_clock::duration &timeout)
    {
        ...
        std::size_t n;
        asio::error_code ec;
        asio::async_read(m_socket, asio::dynamic_buffer(m_stringBuffer),
    ...
    template <class T> void Write(const T &data, const asio::chrono::steady_clock::duration &timeout)
    {
        // Start the asynchronous operation. The boost::lambda function object is
        // used as a callback and will update the ec variable when the operation
        // completes. The blocking_udp_client.cpp example shows how you can use
        // boost::bind rather than boost::lambda.
        asio::error_code ec;
        asio::async_write(m_socket, asio::buffer(data), std::bind(&Client::handle_connect, std::placeholders::_1, &ec));
  ...
  private:
    ...
    static void handle_receive(const asio::error_code &ec, std::size_t length, asio::error_code *out_ec,
