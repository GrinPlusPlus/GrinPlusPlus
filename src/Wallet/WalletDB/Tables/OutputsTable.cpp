...
{
    const std::string tableCreation = "create table outputs(id INTEGER PRIMARY KEY, commitment TEXT UNIQUE NOT NULL, "
                                      "status INTEGER NOT NULL, transaction_id INTEGER, encrypted BLOB NOT NULL);";
    char *error = nullptr;
    if (sqlite3_exec(&database, tableCreation.c_str(), NULL, NULL, &error) != SQLITE_OK)
    {
        LoggerAPI::LogError(
            StringUtil::Format("OutputsTable::CreateTable - Create outputs table failed with error: %s", error));
        sqlite3_free(error);
...
void OutputsTable::UpdateSchema(sqlite3 &database, const SecureVector &masterSeed, const int previousVersion)
{
    ...
    const std::string tableCreation =
        "create table new_outputs(id INTEGER PRIMARY KEY, commitment TEXT UNIQUE NOT NULL, status INTEGER NOT NULL, "
        "transaction_id INTEGER, encrypted BLOB NOT NULL);";
    char *error = nullptr;
    if (sqlite3_exec(&database, tableCreation.c_str(), NULL, NULL, &error) != SQLITE_OK)
    {
        LoggerAPI::LogError(
            StringUtil::Format("OutputsTable::UpdateSchema - Create new_outputs table failed with error: %s", error));
        sqlite3_free(error);
    ...
    {
        LoggerAPI::LogError(
            StringUtil::Format("OutputsTable::UpdateSchema - Dropping outputs table failed with error: %s", error));
        sqlite3_free(error);
    ...
    {
        LoggerAPI::LogError(
            StringUtil::Format("OutputsTable::UpdateSchema - Renaming new_outputs table failed with error: %s", error));
        sqlite3_free(error);
                              ...
                              const std::string &tableName)
{
    for (const OutputData &output : outputs)
    {
        ...
        std::string insert =
            "insert into " + tableName + "(commitment, status, transaction_id, encrypted) values(?, ?, ?, ?)";
        insert += " ON CONFLICT(commitment) DO UPDATE SET status=excluded.status, "
                  "transaction_id=excluded.transaction_id, encrypted=excluded.encrypted";
        if (sqlite3_prepare_v2(&database, insert.c_str(), -1, &stmt, NULL) != SQLITE_OK)
        {
            LoggerAPI::LogError(StringUtil::Format("WalletSqlite::AddOutputs - Error while compiling sql: %s",
        ...
        const std::vector<unsigned char> encrypted =
            WalletEncryptionUtil::Encrypt(masterSeed, "OUTPUT", serializer.GetSecureBytes());
        ...
        {
            LoggerAPI::LogError(StringUtil::Format("WalletSqlite::AddOutputs - Error finalizing statement: %s",
                                                   sqlite3_errmsg(&database)));
...
std::vector<OutputData> OutputsTable::GetOutputs(sqlite3 &database, const SecureVector &masterSeed, const int version)
{
    ...
    {
        LoggerAPI::LogError(StringUtil::Format("SqliteUpdater::ConvertOutputs - Error while compiling sql: %s",
                                               sqlite3_errmsg(&database)));
        sqlite3_finalize(stmt);
        throw WALLET_STORE_EXCEPTION("Error compiling statement.");
    }
    ...
    {
        LoggerAPI::LogError(
            StringUtil::Format("OutputsTable::GetOutputs - Error while performing sql: %s", sqlite3_errmsg(&database)));
    }

    if (sqlite3_finalize(stmt) != SQLITE_OK)
    {
        LoggerAPI::LogError(
            StringUtil::Format("OutputsTable::GetOutputs - Error finalizing statement: %s", sqlite3_errmsg(&database)));
        throw WALLET_STORE_EXCEPTION("Error finalizing statement.");
    }
