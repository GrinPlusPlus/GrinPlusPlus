...
bool BlockSyncer::IsBlockSyncDue(const SyncStatus &syncStatus)
{
    ...
    {
        LoggerAPI::LogTrace(StringUtil::Format("BlockSyncer::IsBlockSyncDue() - %llu blocks received since last check.",
                                               blocksDownloaded));
    }
    ...
    for (auto iter = blocksNeeded.cbegin(); iter != blocksNeeded.cend(); iter++)
    {
        ...
        {
            LoggerAPI::LogTrace(
                "BlockSyncer::IsBlockSyncDue() - Waiting on block from banned peer. Requesting from valid peer now.");
            return true;
        }
...
bool BlockSyncer::RequestBlocks()
{
    ...
    {
        LoggerAPI::LogTrace(StringUtil::Format("BlockSyncer::RequestBlocks - %llu blocks requested from %llu peers.",
                                               blocksToRequest.size(), numPeers));
    }
