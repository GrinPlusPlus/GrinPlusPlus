...
class TransactionKernelMessage : public IMessage
{
  public:
    ...
    // Constructors
    //
    TransactionKernelMessage(Hash &&kernelHash) : m_kernelHash(std::move(kernelHash))
    {
    }
    TransactionKernelMessage(const Hash &kernelHash) : m_kernelHash(kernelHash)
    {
    }
    ...
    // Clone
    //
    virtual TransactionKernelMessage *Clone() const override final
    {
        return new TransactionKernelMessage(*this);
    }
    ...
    {
        return MessageTypes::TransactionKernelMsg;
    }
    inline const Hash &GetKernelHash() const
    {
        return m_kernelHash;
    }
