...
std::vector<Transaction> Pool::GetTransactionsByShortId(const Hash &hash, const uint64_t nonce,
...
std::vector<Transaction> Pool::GetExpiredTransactions(const uint16_t embargoSeconds) const
{
    ...
    const std::time_t cutoff =
        std::chrono::system_clock::to_time_t(std::chrono::system_clock::now() - std::chrono::seconds(embargoSeconds));
...
void Pool::ReconcileBlock(const FullBlock &block, const std::unique_ptr<Transaction> &pMemPoolAggTx)
{
    ...
    for (auto &txPoolEntry : m_transactions)
    {
        if (!ShouldEvict_Locked(txPoolEntry.GetTransaction(), block))
        {
            ...
            filteredEntriesByHash.insert(
                std::pair<Hash, TxPoolEntry>(txPoolEntry.GetTransaction().GetHash(), txPoolEntry));
        }
    }

    m_transactions.clear();

    const std::vector<Transaction> validTransactions =
        ValidTransactionFinder(m_txHashSetManager).FindValidTransactions(filteredTransactions, pMemPoolAggTx);
