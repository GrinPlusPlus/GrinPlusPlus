...
{
public:
	WalletSummary(const uint64_t lastConfirmedHeight, const uint64_t minimumConfirmations, const uint64_t unconfirmed, const uint64_t immature, const uint64_t locked, const uint64_t spendable)
	...
	static WalletSummary FromJSON(const Json::Value& json)
	{
		...
		return WalletSummary(lastConfirmedHeight, minimumConfirmations, unconfirmed, immature, locked, spendable);
	}
