...
std::unique_ptr<PublicExtKey> PublicKeyCalculator::DeterminePublicKey(const PrivateExtKey &privateKey) const
{
    ...
    if (pCompressedPublicKey != nullptr)
    {
        ...
        return std::make_unique<PublicExtKey>(privateKey.GetNetwork(), privateKey.GetDepth(),
                                              privateKey.GetParentFingerprint(), privateKey.GetChildNumber(),
                                              std::move(chainCode), std::move(compressedBytes));
    }
