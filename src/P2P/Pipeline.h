...
class Pipeline
{
  ...
  private:
    ...
    struct BlockEntry
    {
        BlockEntry(const uint64_t connId, const FullBlock &fullBlock) : connectionId(connId), block(fullBlock)
        {
        }
    ...
    // TxHashSet
    static void Thread_ProcessTxHashSet(Pipeline &pipeline, const uint64_t connectionId, const Hash blockHash,
