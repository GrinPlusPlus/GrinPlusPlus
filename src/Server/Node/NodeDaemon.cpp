...
NodeDaemon::NodeDaemon(const Config &config) : m_config(config)
{
}
...
INodeClient *NodeDaemon::Initialize()
{
    ...
    m_pBlockChainServer =
        BlockChainAPI::StartBlockChainServer(m_config, *m_pDatabase, *m_pTxHashSetManager, *m_pTransactionPool);
    m_pP2PServer = P2PAPI::StartP2PServer(m_config, *m_pBlockChainServer, *m_pDatabase, *m_pTransactionPool);

    m_pNodeRestServer =
        new NodeRestServer(m_config, m_pDatabase, m_pTxHashSetManager, m_pBlockChainServer, m_pP2PServer);
    m_pNodeRestServer->Initialize();

    m_pNodeClient = new DefaultNodeClient(*m_pBlockChainServer, m_pDatabase->GetBlockDB(), *m_pTxHashSetManager,
                                          *m_pTransactionPool);
