...
class TxHashSet : public ITxHashSet
{
  public:
    ...
    {
        m_txHashSetMutex.lock_shared();
    }
    inline void Unlock()
    {
        m_txHashSetMutex.unlock_shared();
    }
    inline const BlockHeader &GetBlockHeader() const
    {
        return m_blockHeader;
    }
    inline const BlockHeader &GetFlushedBlockHeader() const
    {
        return m_blockHeaderBackup;
    }
    ...
    virtual std::unique_ptr<BlockSums> ValidateTxHashSet(const BlockHeader &header,
    ...
    virtual OutputRange GetOutputsByLeafIndex(const uint64_t startIndex,
                                              const uint64_t maxNumOutputs) const override final;
    virtual std::vector<OutputDisplayInfo> GetOutputsByMMRIndex(const uint64_t startIndex,
                                                                const uint64_t lastIndex) const override final;
    ...
    KernelMMR *GetKernelMMR()
    {
        return m_pKernelMMR;
    }
    OutputPMMR *GetOutputPMMR()
    {
        return m_pOutputPMMR;
    }
    RangeProofPMMR *GetRangeProofPMMR()
    {
        return m_pRangeProofPMMR;
    }
