...
{
  public:
    OutputRange(const uint64_t highestIndex, const uint64_t lastRetrievedIndex,
    ...
    inline uint64_t GetHighestIndex() const
    {
        return m_highestIndex;
    }
    inline uint64_t GetLastRetrievedIndex() const
    {
        return m_lastRetrievedIndex;
    }
    inline const std::vector<OutputDisplayInfo> &GetOutputs() const
    {
        return m_outputs;
    }
