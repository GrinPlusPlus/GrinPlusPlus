...
std::unique_ptr<Commitment> Crypto::AddCommitments(const std::vector<Commitment> &positive,
...
std::unique_ptr<BlindingFactor> Crypto::AddBlindingFactors(const std::vector<BlindingFactor> &positive,
...
std::unique_ptr<RangeProof> Crypto::GenerateRangeProof(const uint64_t amount, const SecretKey &key,
                                                       const SecretKey &privateNonce, const SecretKey &rewindNonce,
...
std::unique_ptr<RewoundProof> Crypto::RewindRangeProof(const Commitment &commitment, const RangeProof &rangeProof,
...
std::vector<unsigned char> Crypto::AES256_Encrypt(const SecureVector &input, const SecretKey &key,
...
SecureVector Crypto::AES256_Decrypt(const std::vector<unsigned char> &ciphertext, const SecretKey &key,
                                    const CBigInteger<16> &iv)
{
                        ...
                        const ScryptParameters &parameters)
{
    ...
    if (crypto_scrypt((const unsigned char *)password.data(), password.size(), salt.data(), salt.size(), parameters.N,
                      parameters.r, parameters.p, buffer.data(), buffer.size()) == 0)
    {
        std::vector<unsigned char> tmp(32, 0);
...
std::unique_ptr<CompactSignature> Crypto::SignMessage(const SecretKey &secretKey, const PublicKey &publicKey,
...
bool Crypto::VerifyMessageSignature(const CompactSignature &signature, const PublicKey &publicKey,
                                    const std::string &message)
{
    const Hash messageHash = Crypto::Blake2b(std::vector<unsigned char>(message.cbegin(), message.cend()));
...
std::unique_ptr<CompactSignature> Crypto::CalculatePartialSignature(const SecretKey &secretKey,
...
std::unique_ptr<Signature> Crypto::AggregateSignatures(const std::vector<CompactSignature> &signatures,
                                                       const PublicKey &sumPubNonces)
{
    return AggSig::GetInstance().AggregateSignatures(signatures, sumPubNonces);
}

bool Crypto::VerifyPartialSignature(const CompactSignature &partialSignature, const PublicKey &publicKey,
                                    const PublicKey &sumPubKeys, const PublicKey &sumPubNonces, const Hash &message)
{
    return AggSig::GetInstance().VerifyPartialSignature(partialSignature, publicKey, sumPubKeys, sumPubNonces, message);
}

bool Crypto::VerifyAggregateSignature(const Signature &aggregateSignature, const PublicKey sumPubKeys,
                                      const Hash &message)
{
    return AggSig::GetInstance().VerifyAggregateSignature(aggregateSignature, sumPubKeys, message);
}

bool Crypto::VerifyKernelSignatures(const std::vector<const Signature *> &signatures,
