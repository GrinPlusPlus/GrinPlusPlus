...
std::unique_ptr<Slate> ReceiveSlateBuilder::AddReceiverData(Wallet &wallet, const SecureVector &masterSeed,
                                                            const Slate &slate,
                                                            ...
                                                            const std::optional<std::string> &messageOpt) const
{
    ...
    OutputData outputData = wallet.CreateBlindedOutput(masterSeed, receiveSlate.GetAmount(), walletTxId,
                                                       EBulletproofType::ENHANCED, messageOpt);
    ...
    receiveSlate.UpdateTransaction(
        TransactionBuilder::AddOutput(receiveSlate.GetTransaction(), outputData.GetOutput()));
...
bool ReceiveSlateBuilder::VerifySlateStatus(Wallet &wallet, const SecureVector &masterSeed, const Slate &slate) const
{
    ...
    {
        LoggerAPI::LogError("ReceiveSlateBuilder::VerifySlateStatus - Already received slate " +
    ...
    {
        LoggerAPI::LogError("ReceiveSlateBuilder::VerifySlateStatus - Failed to verify message signatures for slate " +
                            uuids::to_string(slate.GetSlateId()));
        return false;
    }
    ...
    if (kernels.size() != 1)
    {
        LoggerAPI::LogError("ReceiveSlateBuilder::VerifySlateStatus - Slate " + uuids::to_string(slate.GetSlateId()) +
                            " had " + std::to_string(kernels.size()) + " kernels.");
        return false;
    }
...
void ReceiveSlateBuilder::AddParticipantData(Slate &slate, const SecretKey &secretKey, const SecretKey &secretNonce,
                                             const std::optional<std::string> &messageOpt) const
{
    ...
    std::unique_ptr<CompactSignature> pPartialSignature =
        SignatureUtil::GeneratePartialSignature(secretKey, secretNonce, participants, kernelMessage);
    if (pPartialSignature == nullptr)
    {
        LoggerAPI::LogError("SlateBuilder::AddParticipantData - Failed to generate signature for slate " +
                            uuids::to_string(slate.GetSlateId()));
        throw CryptoException();
    }
    ...
    if (messageOpt.has_value())
    {
        ...
        {
            LoggerAPI::LogError("SlateBuilder::AddParticipantData - Failed to sign message for slate " +
                                uuids::to_string(slate.GetSlateId()));
            throw CryptoException();
        }
    ...
    {
        LoggerAPI::LogError("SlateBuilder::AddParticipantData - Failed to verify signature for slate " +
                            uuids::to_string(slate.GetSlateId()));
        throw CryptoException();
    }
}
...
// TODO: Use a DB Batch
bool ReceiveSlateBuilder::UpdateDatabase(Wallet &wallet, const SecureVector &masterSeed, const Slate &slate,
                                         const OutputData &outputData, const uint32_t walletTxId,
                                         const SlateContext &context, const std::optional<std::string> &addressOpt,
                                         const std::optional<std::string> &messageOpt) const
{
    // Save secretKey and secretNonce
    if (!wallet.SaveSlateContext(slate.GetSlateId(), masterSeed, context))
    {
        LoggerAPI::LogError("ReceiveSlateBuilder::UpdateDatabase - Failed to save context for slate " +
                            uuids::to_string(slate.GetSlateId()));
        return false;
    }
    ...
    {
        LoggerAPI::LogError("ReceiveSlateBuilder::UpdateDatabase - Failed to save output for slate " +
                            uuids::to_string(slate.GetSlateId()));
        return false;
    }

    // Save WalletTx
    WalletTx walletTx(walletTxId, EWalletTxType::RECEIVING_IN_PROGRESS,
                      ...
                      std::chrono::system_clock::now(), std::nullopt, std::nullopt, slate.GetAmount(), 0,
                      std::optional<uint64_t>(slate.GetFee()), std::nullopt);
