...
bool Zipper::CreateZipFile(const std::string& destination, const std::vector<std::string>& paths)
{
	...
	{
		if (ghc::filesystem::is_directory(ghc::filesystem::path(paths[i])))
		{
...
bool Zipper::AddDirectory(zipFile zf, const ghc::filesystem::path& sourceDir, const std::string& destDir)
{
	for (const auto& entry : ghc::filesystem::directory_iterator(sourceDir))
	{
