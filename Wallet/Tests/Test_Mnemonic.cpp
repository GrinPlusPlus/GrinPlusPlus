...
{
	{
		...
		SecureString mnemonic = Mnemonic().CreateMnemonic(entropy, std::nullopt);
...
{
	{
		...
		std::optional<std::vector<unsigned char>> entropyOpt = Mnemonic().ToEntropy(walletWords, std::nullopt);
		std::vector<unsigned char> expected = CBigInteger<16>::FromHex("00000000000000000000000000000000").GetData();
		REQUIRE(entropyOpt.value() == expected);
	}
