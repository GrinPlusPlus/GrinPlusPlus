...
Wallet::Wallet(const Config &config, const INodeClient &nodeClient, IWalletDB &walletDB, const std::string &username,
               KeyChainPath &&userPath)
    : m_config(config), m_nodeClient(nodeClient), m_walletDB(walletDB), m_username(username),
      m_userPath(std::move(userPath))
{
}

Wallet *Wallet::LoadWallet(const Config &config, const INodeClient &nodeClient, IWalletDB &walletDB,
...
WalletSummary Wallet::GetWalletSummary(const SecureVector &masterSeed)
{
    ...
    return WalletSummary(lastConfirmedHeight, m_config.GetWalletConfig().GetMinimumConfirmations(), total,
                         awaitingConfirmation, immature, locked, spendable, std::move(transactions));
}
...
bool Wallet::AddRestoredOutputs(const SecureVector &masterSeed, const std::vector<OutputData> &outputs)
{
    ...
    for (const OutputData &output : outputs)
    {
        ...
        const std::chrono::system_clock::time_point creationTime =
            std::chrono::system_clock::now(); // TODO: Determine this
        ...
        WalletTx walletTx(walletTxId, type, std::nullopt, std::nullopt, std::nullopt, creationTime, confirmationTimeOpt,
                          output.GetBlockHeight(), output.GetAmount(), 0, std::nullopt, std::nullopt);
                                       ...
                                       const std::optional<std::string> &messageOpt)
{
    ...
    std::unique_ptr<Commitment> pCommitment = Crypto::CommitBlinded(
        amount, BlindingFactor(blindingFactor.GetBytes())); // TODO: Creating a BlindingFactor here is unsafe. The
                                                            // memory may not get cleared.
    if (pCommitment != nullptr)
    {
        std::unique_ptr<RangeProof> pRangeProof =
            keyChain.GenerateRangeProof(keyChainPath, amount, *pCommitment, blindingFactor, bulletproofType);
        if (pRangeProof != nullptr)
        {
            TransactionOutput transactionOutput(EOutputFeatures::DEFAULT_OUTPUT, Commitment(*pCommitment),
                                                RangeProof(*pRangeProof));

            return OutputData(std::move(keyChainPath), std::move(blindingFactor), std::move(transactionOutput), amount,
                              EOutputStatus::NO_CONFIRMATIONS, std::make_optional<uint32_t>(walletTxId), messageOpt);
        }
    }
...
bool Wallet::SaveSlateContext(const uuids::uuid &slateId, const SecureVector &masterSeed,
                              const SlateContext &slateContext)
{
