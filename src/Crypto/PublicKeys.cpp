...
std::unique_ptr<PublicKey> PublicKeys::CalculatePublicKey(const SecretKey &privateKey) const
{
    ...
    if (verifyResult == 1)
    {
        ...
        if (createResult == 1)
        {
            ...
            const int serializeResult = secp256k1_ec_pubkey_serialize(m_pContext, serializedPublicKey.data(), &length,
...
std::unique_ptr<PublicKey> PublicKeys::PublicKeySum(const std::vector<PublicKey> &publicKeys) const
{
    ...
    const int pubKeysCombined =
        secp256k1_ec_pubkey_combine(m_pContext, &publicKey, parsedPubKeys.data(), parsedPubKeys.size());
