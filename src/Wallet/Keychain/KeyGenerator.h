...
//
// For specifications on generating the master key for HD wallets, see
...
class KeyGenerator
{
  public:
    ...
    //
    std::unique_ptr<PrivateExtKey> GenerateChildPrivateKey(const PrivateExtKey &parentExtendedKey,
    ...
    //
    PublicExtKey GenerateChildPublicKey(const PublicExtKey &parentExtendedPublicKey,
                                        const uint32_t childKeyIndex) const;
