...
class GetHeadersMessage : public IMessage
{
  public:
    ...
    // Constructors
    //
    GetHeadersMessage(std::vector<CBigInteger<32>> &&hashes) : m_hashes(std::move(hashes))
    {
    }
    ...
    // Clone
    //
    virtual GetHeadersMessage *Clone() const override final
    {
    ...
    {
        return MessageTypes::GetHeaders;
    }
    inline const std::vector<CBigInteger<32>> &GetHashes() const
    {
        return m_hashes;
    }
