...
bool Zipper::AddFile(zipFile zf, const std::string &sourceFile, const std::string &destDir)
{
    ...
    if (file.is_open())
    {
        ...
        if (size == 0 || file.read(&buffer[0], size))
        {
            zip_fileinfo zfi = {0};
            std::string fileName =
                destDir + "/" + sourceFile.substr(std::max(sourceFile.rfind('\\'), sourceFile.rfind('/')) + 1);

            if (ZIP_OK ==
                zipOpenNewFileInZip(zf, fileName.c_str(), &zfi, nullptr, 0, nullptr, 0, nullptr, 0, Z_NO_COMPRESSION))
            {
