...
// A 'Slate' is passed around to all parties to build up all of the public transaction data needed to create a finalized
// transaction. Callers can pass the slate around by whatever means they choose, (but we can provide some binary or JSON
// serialization helpers here).
class Slate
{
  public:
    Slate(SlateVersionInfo &&versionInfo, const uint64_t numParticipants, uuids::uuid &&slateId,
          Transaction &&transaction, const uint64_t amount, const uint64_t fee, const uint64_t blockHeight,
          const uint64_t lockHeight)
        : m_versionInfo(versionInfo), m_numParticipants(numParticipants), m_slateId(std::move(slateId)),
          m_transaction(std::move(transaction)), m_amount(amount), m_fee(fee), m_blockHeight(blockHeight),
          m_lockHeight(lockHeight)
    {
    }

    inline const uuids::uuid &GetSlateId() const
    {
        return m_slateId;
    }
    inline const SlateVersionInfo &GetVersionInfo() const
    {
    ...
    inline uint64_t GetFee() const
    {
        return m_fee;
    }
    inline uint64_t GetLockHeight() const
    {
        return m_lockHeight;
    }
    ...
    inline const std::vector<ParticipantData> &GetParticipantData() const
    {
        return m_participantData;
    }
    inline std::vector<ParticipantData> &GetParticipantData()
    {
        return m_participantData;
    }

    inline void AddParticpantData(const ParticipantData &participantData)
    {
        m_participantData.push_back(participantData);
    }
    inline void UpdateTransaction(const Transaction &transaction)
    {
    ...
    static Slate FromJSON(const Json::Value &slateNode)
    {
        ...
        SlateVersionInfo versionInfo = (versionInfoJSON != Json::nullValue)
                                           ? SlateVersionInfo::FromJSON(versionInfoJSON)
                                           : SlateVersionInfo(version);
        ...
        std::optional<uuids::uuid> slateIdOpt =
            uuids::uuid::from_string(JsonUtil::GetRequiredField(slateNode, "id").asString());
        ...
        Slate slate(std::move(versionInfo), numParticipants, std::move(slateIdOpt.value()), std::move(transaction),
                    amount, fee, height, lockHeight);
