...
HandShake::HandShake(const Config &config, ConnectionManager &connectionManager, PeerManager &peerManager,
...
bool HandShake::PerformHandshake(Socket &socket, ConnectedPeer &connectedPeer, const EDirection direction) const
{
    try
    {
        LoggerAPI::LogTrace("HandShake::PerformHandshake - Performing handshake with " +
                            socket.GetSocketAddress().Format() + std::string(" - direction: ") +
    ...
    {
        LoggerAPI::LogDebug("HandShake::PerformHandshake - Failed to deserialize handshake from " +
                            socket.GetSocketAddress().Format());
    }
    catch (const SocketException &)
    {
        LoggerAPI::LogDebug("HandShake::PerformHandshake - Socket exception encountered with " +
                            socket.GetSocketAddress().Format());
    }
...
bool HandShake::PerformOutboundHandshake(Socket &socket, ConnectedPeer &connectedPeer) const
{
    ...
    {
        // Get Shake Message
        std::unique_ptr<RawMessage> pReceivedMessage =
            MessageRetriever(m_config, m_connectionManager)
                .RetrieveMessage(socket, connectedPeer, MessageRetriever::BLOCKING);
        ...
        if (pReceivedMessage.get() != nullptr)
        {
            ...
            else if (messageType == MessageTypes::BanReasonMsg)
            {
                ...
                LoggerAPI::LogDebug("HandShake::PerformOutboundHandshake - Ban message received from " +
                                    socket.GetSocketAddress().Format() +
                                    " with reason: " + std::to_string(banReasonMessage.GetBanReason()));
                return false;
            }
            else
            {
                LoggerAPI::LogDebug("HandShake::PerformOutboundHandshake - Expected shake from " +
                                    socket.GetSocketAddress().Format() + " but received " +
                                    MessageTypes::ToString(messageType));
                return false;
            }
        }

        LoggerAPI::LogTrace("HandShake::PerformOutboundHandshake - Shake message not received from " +
                            socket.GetSocketAddress().Format());
        return false;
    }

    LoggerAPI::LogDebug("HandShake::PerformOutboundHandshake - Hand message not sent to " +
                        socket.GetSocketAddress().Format());
    return false;
}
...
bool HandShake::PerformInboundHandshake(Socket &socket, ConnectedPeer &connectedPeer) const
{
    ...
    if (pReceivedMessage != nullptr)
    {
        if (pReceivedMessage->GetMessageHeader().GetMessageType() == MessageTypes::Hand)
        {
            ...
            if (handMessage.GetNonce() != NONCE &&
                !m_connectionManager.IsConnected(connectedPeer.GetPeer().GetIPAddress()))
            {
                ...
                {
                    LoggerAPI::LogDebug("HandShake::PerformInboundHandshake - Failed to transmit shake message to " +
                                        socket.GetSocketAddress().Format());
                    return false;
                }
            }
            ...
            {
                LoggerAPI::LogDebug("HandShake::PerformInboundHandshake - Connected to self(" +
                                    socket.GetSocketAddress().Format() + "). Nonce: " + std::to_string(NONCE));
            }
            else
            {
                LoggerAPI::LogDebug("HandShake::PerformInboundHandshake - Already connected to " +
                                    connectedPeer.GetPeer().GetIPAddress().Format());
            }
        }
        else
        {
            LoggerAPI::LogDebug("HandShake::PerformInboundHandshake - First message from " +
                                socket.GetSocketAddress().Format() + " was of type " +
                                std::to_string(pReceivedMessage->GetMessageHeader().GetMessageType()));
            return false;
        }
    }

    LoggerAPI::LogTrace("HandShake::PerformInboundHandshake - Unable to connect to " +
                        socket.GetSocketAddress().Format());
    return false;
}
...
bool HandShake::TransmitHandMessage(Socket &socket) const
{
    ...
    const Capabilities capabilities(
        Capabilities::FAST_SYNC_NODE); // LIGHT_CLIENT: Read P2P Config once light-clients are supported
    ...
    const HandMessage handMessage(version, capabilities, nonce, std::move(hash), totalDifficulty,
                                  std::move(senderAddress), std::move(receiverAddress), userAgent);
