...
std::unique_ptr<CompactSignature> AggSig::SignMessage(const SecretKey &secretKey, const PublicKey &publicKey,
                                                      const Hash &message)
{
    ...
    if (pubKeyParsed == 1)
    {
        ...
        const int signedResult =
            secp256k1_aggsig_sign_single(m_pContext, &signature.data[0], message.data(), secretKey.data(), nullptr,
                                         nullptr, nullptr, nullptr, &pubKey, randomSeed.data());

        if (signedResult == 1)
        {
            std::vector<unsigned char> signatureBytes(64);
            const int serializedResult =
                secp256k1_ecdsa_signature_serialize_compact(m_pContext, signatureBytes.data(), &signature);
...
bool AggSig::VerifyMessageSignature(const CompactSignature &signature, const PublicKey &publicKey,
                                    const Hash &message) const
{
    std::shared_lock<std::shared_mutex> readLock(m_mutex);

    secp256k1_ecdsa_signature secpSig;
    const int parseSignatureResult =
        secp256k1_ecdsa_signature_parse_compact(m_pContext, &secpSig, signature.GetSignatureBytes().data());
    if (parseSignatureResult == 1)
    {
        ...
        if (pubkeyResult == 1)
        {
            const int verifyResult = secp256k1_aggsig_verify_single(m_pContext, secpSig.data, message.data(), nullptr,
                                                                    &pubkey, &pubkey, nullptr, false);
            if (verifyResult == 1)
            {
                return true;
            }
...
std::unique_ptr<CompactSignature> AggSig::CalculatePartialSignature(const SecretKey &secretKey,
                                                                    const SecretKey &secretNonce,
                                                                    const PublicKey &sumPubKeys,
                                                                    const PublicKey &sumPubNonces, const Hash &message)
{
    std::lock_guard<std::shared_mutex> writeLock(m_mutex);
    ...
    if (pubKeyParsed == 1 && noncesParsed == 1)
    {
        ...
        const int signedResult = secp256k1_aggsig_sign_single(
            m_pContext, &signature.data[0], message.data(), secretKey.data(), secretNonce.data(), nullptr,
            &pubNoncesForE, &pubNoncesForE, &pubKeyForE, randomSeed.data());
...
bool AggSig::VerifyPartialSignature(const CompactSignature &partialSignature, const PublicKey &publicKey,
                                    const PublicKey &sumPubKeys, const PublicKey &sumPubNonces,
                                    const Hash &message) const
{
    std::shared_lock<std::shared_mutex> readLock(m_mutex);
    ...
    const int parseSignatureResult =
        secp256k1_ecdsa_signature_parse_compact(m_pContext, &signature, partialSignature.GetSignatureBytes().data());
    if (parseSignatureResult == 1)
    {
        secp256k1_pubkey pubkey;
        const int pubkeyResult = secp256k1_ec_pubkey_parse(m_pContext, &pubkey, publicKey.data(), publicKey.size());
        ...
        const int sumPubkeysResult =
            secp256k1_ec_pubkey_parse(m_pContext, &sumPubKey, sumPubKeys.data(), sumPubKeys.size());

        secp256k1_pubkey sumNoncesPubKey;
        const int sumPubNonceKeyResult =
            secp256k1_ec_pubkey_parse(m_pContext, &sumNoncesPubKey, sumPubNonces.data(), sumPubNonces.size());

        if (pubkeyResult == 1 && sumPubkeysResult == 1 && sumPubNonceKeyResult == 1)
        {
            const int verifyResult = secp256k1_aggsig_verify_single(
                m_pContext, signature.data, message.data(), &sumNoncesPubKey, &pubkey, &sumPubKey, nullptr, true);
            if (verifyResult == 1)
            {
                return true;
            }
...
std::unique_ptr<Signature> AggSig::AggregateSignatures(const std::vector<CompactSignature> &signatures,
                                                       const PublicKey &sumPubNonces) const
{
    std::shared_lock<std::shared_mutex> readLock(m_mutex);

    secp256k1_pubkey pubNonces;
    const int noncesParsed =
        secp256k1_ec_pubkey_parse(m_pContext, &pubNonces, sumPubNonces.data(), sumPubNonces.size());
    if (noncesParsed == 1)
    {
        ...
        if (!parsedSignatures.empty())
        {
            ...
            const int result = secp256k1_aggsig_add_signatures_single(m_pContext, aggregatedSignature.data,
                                                                      (const unsigned char **)signaturePointers.data(),
                                                                      signaturePointers.size(), &pubNonces);
...
bool AggSig::VerifyAggregateSignatures(const std::vector<const Signature *> &signatures,
                                       ...
                                       const std::vector<const Hash *> &messages) const
{
    ...
    for (const Commitment *commitment : commitments)
    {
        ...
        const int commitmentResult =
            secp256k1_pedersen_commitment_parse(m_pContext, &parsedCommitment, commitment->GetCommitmentBytes().data());
    ...
    const int verifyResult = secp256k1_schnorrsig_verify_batch(
        m_pContext, pScratchSpace, signaturePtrs.data(), messageData.data(), pubKeyPtrs.data(), signatures.size());
...
bool AggSig::VerifyAggregateSignature(const Signature &signature, const PublicKey &sumPubKeys,
                                      const Hash &message) const
{
    std::shared_lock<std::shared_mutex> readLock(m_mutex);
    ...
    if (parseResult == 1)
    {
        const int verifyResult =
            secp256k1_aggsig_verify_single(m_pContext, signature.GetSignatureBytes().data(), message.data(), nullptr,
                                           &parsedPubKey, &parsedPubKey, nullptr, false);
        if (verifyResult == 1)
        {
            return true;
        }
    ...
    const std::vector<CompactSignature> &signatures) const
{
    ...
    for (const Signature partialSignature : signatures)
    {
        ...
        const int parseSignatureResult = secp256k1_ecdsa_signature_parse_compact(
            m_pContext, &signature, partialSignature.GetSignatureBytes().data());
        if (parseSignatureResult == 1)
        {
