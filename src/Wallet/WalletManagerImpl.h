...
class WalletManager : public IWalletManager
{
  public:
    ...
    virtual std::optional<std::pair<SecureString, SessionToken>> InitializeNewWallet(
        const std::string &username, const SecureString &password) override final;

    virtual std::optional<SessionToken> RestoreFromSeed(const std::string &username, const SecureString &password,
                                                        const SecureString &walletWords) override final;
    ...
    virtual std::unique_ptr<SessionToken> Login(const std::string &username,
                                                const SecureString &password) override final;
    ...
    virtual std::vector<WalletOutputDTO> GetOutputs(const SessionToken &token, const bool includeSpent,
                                                    const bool includeCanceled) override final;
    ...
    virtual FeeEstimateDTO EstimateFee(const SessionToken &token, const uint64_t amountToSend, const uint64_t feeBase,
                                       ...
                                       const uint8_t numChangeOutputs) override final;
    ...
    virtual std::unique_ptr<Slate> Receive(const SessionToken &token, const Slate &slate,
                                           ...
                                           const std::optional<std::string> &messageOpt) override final;
