...
class PeerManager
{
  ...
  private:
    struct PeerEntry
    {
        PeerEntry(Peer &&peer) : m_peer(std::move(peer)), m_lastAttempt(0), m_connected(false), m_dirty(false)
        {
        }
    ...
    std::vector<Peer> GetPeersWithCapability(const Capabilities::ECapability &preferredCapability,
