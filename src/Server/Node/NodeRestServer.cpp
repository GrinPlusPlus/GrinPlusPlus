...
NodeRestServer::NodeRestServer(const Config &config, IDatabase *pDatabase, TxHashSetManager *pTxHashSetManager,
                               IBlockChainServer *pBlockChainServer, IP2PServer *pP2PServer)
    : m_config(config), m_pDatabase(pDatabase), m_pTxHashSetManager(pTxHashSetManager),
...
bool NodeRestServer::Initialize()
{
    ...
    const char *mg_options[] = {"num_threads", "5", "listening_ports", std::to_string(port).c_str(), NULL};
    m_pNodeCivetContext = mg_start(NULL, 0, mg_options);

    /* Add handlers */
    mg_set_request_handler(m_pNodeCivetContext, "/v1/explorer/blockinfo/", BlockInfoAPI::GetBlockInfo_Handler,
                           m_pNodeContext);
    ...
    mg_set_request_handler(m_pNodeCivetContext, "/v1/chain/outputs/byids", ChainAPI::GetChainOutputsByIds_Handler,
                           m_pNodeContext);
    mg_set_request_handler(m_pNodeCivetContext, "/v1/chain/outputs/byheight", ChainAPI::GetChainOutputsByHeight_Handler,
                           m_pNodeContext);
    ...
    mg_set_request_handler(m_pNodeCivetContext, "/v1/peers/connected", PeersAPI::GetConnectedPeers_Handler,
                           m_pNodeContext);
    ...
    mg_set_request_handler(m_pNodeCivetContext, "/v1/txhashset/lastkernels", TxHashSetAPI::GetLastKernels_Handler,
                           m_pNodeContext);
    mg_set_request_handler(m_pNodeCivetContext, "/v1/txhashset/lastoutputs", TxHashSetAPI::GetLastOutputs_Handler,
                           m_pNodeContext);
    mg_set_request_handler(m_pNodeCivetContext, "/v1/txhashset/lastrangeproofs",
                           TxHashSetAPI::GetLastRangeproofs_Handler, m_pNodeContext);
    mg_set_request_handler(m_pNodeCivetContext, "/v1/txhashset/outputs", TxHashSetAPI::GetOutputs_Handler,
                           m_pNodeContext);
