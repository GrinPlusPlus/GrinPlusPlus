...
TransactionPool::TransactionPool(const Config &config, const TxHashSetManager &txHashSetManager,
                                 const IBlockDB &blockDB)
    : m_config(config), m_txHashSetManager(txHashSetManager), m_blockDB(blockDB),
      m_memPool(config, txHashSetManager, blockDB), m_stemPool(config, txHashSetManager, blockDB)
{
}

std::vector<Transaction> TransactionPool::GetTransactionsByShortId(const Hash &hash, const uint64_t nonce,
                                                                   const std::set<ShortId> &missingShortIds) const
{
...
std::unique_ptr<Transaction> TransactionPool::GetTransactionToStem()
{
    ...
    std::vector<Transaction> validTransactionsToStem =
        ValidTransactionFinder(m_txHashSetManager).FindValidTransactions(transactionsToStem, pMemPoolAggTx);
...
std::unique_ptr<Transaction> TransactionPool::GetTransactionToFluff()
{
    ...
    const std::vector<Transaction> transactionsToFluff =
        m_stemPool.FindTransactionsByStatus(EDandelionStatus::TO_FLUFF);
    ...
    std::vector<Transaction> validTransactionsToFluff =
        ValidTransactionFinder(m_txHashSetManager).FindValidTransactions(transactionsToFluff, pMemPoolAggTx);
...
std::vector<Transaction> TransactionPool::GetExpiredTransactions() const
{
    ...
    const uint16_t embargoSeconds =
        m_config.GetDandelionConfig().GetEmbargoSeconds() + (uint16_t)RandomNumberGenerator::GenerateRandom(0, 30);
