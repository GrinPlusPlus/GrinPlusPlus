...
std::unique_ptr<FullBlock> BlockAPI::GetBlock(const std::string &requestedBlock, IBlockChainServer *pBlockChainServer)
{
    if (requestedBlock.length() == 64 && HexUtil::IsValidHex(requestedBlock))
    {
        try
        {
            ...
            {
                LoggerAPI::LogDebug(
                    StringUtil::Format("BlockAPI::GetBlock - Found block with hash %s.", requestedBlock.c_str()));
                return pBlock;
            }
            else
            {
                LoggerAPI::LogInfo(
                    StringUtil::Format("BlockAPI::GetBlock - No block found with hash %s.", requestedBlock.c_str()));
            }
        }
        catch (const std::exception &)
        {
            LoggerAPI::LogError(
                StringUtil::Format("BlockAPI::GetBlock - Failed converting %s to a Hash.", requestedBlock.c_str()));
        }
    }
    else if (requestedBlock.length() == 66 && HexUtil::IsValidHex(requestedBlock))
    {
        try
        {
            ...
            if (pBlock != nullptr)
            {
                LoggerAPI::LogDebug(StringUtil::Format("BlockAPI::GetBlock - Found block with output commitment %s.",
                                                       requestedBlock.c_str()));
                return pBlock;
            }
            else
            {
                LoggerAPI::LogInfo(StringUtil::Format("BlockAPI::GetBlock - No block found with commitment %s.",
                                                      requestedBlock.c_str()));
            }
        }
        catch (const std::exception &)
        {
            LoggerAPI::LogError(StringUtil::Format("BlockAPI::GetBlock - Failed converting %s to a Commitment.",
                                                   requestedBlock.c_str()));
        }
    }
    else
    {
        try
        {
            ...
            {
                LoggerAPI::LogInfo(
                    StringUtil::Format("BlockAPI::GetBlock - Found block at height %s.", requestedBlock.c_str()));
                return pBlock;
            }
            else
            {
                LoggerAPI::LogInfo(
                    StringUtil::Format("BlockAPI::GetBlock - No block found at height %s.", requestedBlock.c_str()));
            }
        }
        catch (const std::invalid_argument &)
        {
            LoggerAPI::LogError(
                StringUtil::Format("BlockAPI::GetBlock - Failed converting %s to height.", requestedBlock.c_str()));
        }
    }
