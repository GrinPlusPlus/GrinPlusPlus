...
BlockLocator::BlockLocator(IBlockChainServer &blockChainServer) : m_blockChainServer(blockChainServer)
{
}
...
std::vector<CBigInteger<32>> BlockLocator::GetLocators(const SyncStatus &syncStatus) const
{
    ...
    for (const uint64_t locatorHeight : locatorHeights)
    {
        std::unique_ptr<BlockHeader> pHeader =
            m_blockChainServer.GetBlockHeaderByHeight(locatorHeight, EChainType::SYNC);
        if (pHeader != nullptr)
        {
...
std::vector<BlockHeader> BlockLocator::LocateHeaders(const std::vector<CBigInteger<32>> &locatorHashes) const
{
    ...
    if (pCommonHeader != nullptr)
    {
        ...
        {
            std::unique_ptr<BlockHeader> pHeader =
                m_blockChainServer.GetBlockHeaderByHeight(headerHeight + i, EChainType::SYNC);
            if (pHeader == nullptr)
            {
                break;
            }
