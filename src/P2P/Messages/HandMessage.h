...
class HandMessage : public IMessage
{
  public:
    ...
    // Constructors
    //
    HandMessage(const uint32_t version, const Capabilities &capabilities, const uint64_t nonce, CBigInteger<32> &&hash,
                const uint64_t totalDifficulty, SocketAddress &&senderAddress, SocketAddress &&receiverAddress,
                const std::string &userAgent)
        : m_version(version), m_capabilities(capabilities), m_nonce(nonce), m_hash(std::move(hash)),
          m_totalDifficulty(totalDifficulty), m_senderAddress(std::move(senderAddress)),
          m_receiverAddress(std::move(receiverAddress)), m_userAgent(userAgent)
    {
    }
    ...
    // Clone
    //
    virtual HandMessage *Clone() const override final
    {
        return new HandMessage(*this);
    }
    ...
    inline const uint64_t GetNonce() const
    {
        return m_nonce;
    }
    inline const CBigInteger<32> &GetHash() const
    {
        return m_hash;
    }
    inline const uint64_t GetTotalDifficulty() const
    {
        return m_totalDifficulty;
    }
    inline const SocketAddress &GetSenderAddress() const
    {
        return m_senderAddress;
    }
    inline const SocketAddress &GetReceiverAddress() const
    {
    ...
    static HandMessage Deserialize(ByteBuffer &byteBuffer)
    {
        ...
        return HandMessage(version, capabilities, nonce, std::move(hash), totalDifficulty, std::move(senderAddress),
                           std::move(receiverAddress), userAgent);
    }
