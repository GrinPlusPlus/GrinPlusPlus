...
class IBlockChainServer
{
  public:
    ...
    virtual std::unique_ptr<BlockHeader> GetBlockHeaderByHeight(const uint64_t height,
                                                                const EChainType chainType) const = 0;
    ...
    virtual std::vector<BlockWithOutputs> GetOutputsByHeight(const uint64_t startHeight,
                                                             const uint64_t maxHeight) const = 0;

    //
    // Returns the hashes of blocks(indexed by height) that are part of the candidate (header) chain, but whose bodies
