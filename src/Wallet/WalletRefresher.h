...
class WalletRefresher
{
  ...
  private:
    ...
    void RefreshTransactions(const std::string &username, const SecureVector &masterSeed,
                             const std::vector<OutputData> &walletOutputs, std::vector<WalletTx> &walletTransactions);

    std::unique_ptr<OutputData> FindOutput(const std::vector<OutputData> &walletOutputs,
