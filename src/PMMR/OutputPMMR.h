...
public:
	static std::shared_ptr<OutputPMMR> Load(const fs::path& txHashSetPath)
	{
		...
		if (!FileUtil::Exists(txHashSetPath.u8string() + "output/pmmr_leafset.bin") && FileUtil::Exists(txHashSetPath.u8string() + "output/pmmr_leaf.bin"))
		{
			Roaring outputBitmap;
			std::vector<unsigned char> outputBytes;
			if (FileUtil::ReadFile(txHashSetPath.u8string() + "output/pmmr_leaf.bin", outputBytes))
			{
				outputBitmap = Roaring::readSafe((const char*)outputBytes.data(), outputBytes.size());
			}
			BitmapFile::Create(txHashSetPath.u8string() + "output/pmmr_leafset.bin", outputBitmap);
		}
