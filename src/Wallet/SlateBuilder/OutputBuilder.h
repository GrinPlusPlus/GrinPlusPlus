...
{
  public:
    static std::vector<OutputData> CreateOutputs(Wallet &wallet, const SecureVector &masterSeed,
                                                 const uint64_t totalAmount, const uint32_t walletTxId,
                                                 const uint8_t numOutputs, const EBulletproofType &bulletproofType,
                                                 const std::optional<std::string> &messageOpt)
    {
        ...
        for (uint8_t i = 0; i < numOutputs; i++)
        {
            ...
            outputs.emplace_back(
                wallet.CreateBlindedOutput(masterSeed, coinAmount, walletTxId, bulletproofType, messageOpt));
        }
